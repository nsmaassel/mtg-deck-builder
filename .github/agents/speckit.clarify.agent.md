...
3. Before finalizing the ambiguity scan, grill the spec against the repo's current language and documented reality:

   - Read relevant existing feature docs, ADRs, and `docs/memory/*.memory.md` files when the feature uses established domain language.
   - Challenge synonyms, overloaded terms, and claims that appear inconsistent with existing code, docs, or operating assumptions.
   - Prioritize clarification questions that establish shared vocabulary or expose code-vs-spec contradictions before lower-impact polish questions.
